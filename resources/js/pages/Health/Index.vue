...
<template>
    ...
    <SecurityAdminLayout>
        ...
        <template #description
            >Engine state, the active pipeline, and the PHP extensions it
            depends on.</template
        >
        ...
        <div class="mt-6 grid gap-5 lg:grid-cols-3">
            <section
                class="rounded-lg border border-slate-200 bg-white p-4 lg:col-span-2 dark:border-slate-800 dark:bg-slate-900"
            >
                <h2 class="text-sm font-semibold">Active pipeline</h2>
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    Checks run in this order. Cheap structural checks come first
                    so an obviously bad upload never reaches the malware engine.
                </p>
                ...
                <div class="mt-4">
                    ...
                    <ol class="flex flex-wrap items-center gap-1.5">
                        <li
                            v-for="(check, index) in props.pipeline.file"
                            :key="check.key"
                            class="flex items-center gap-1.5"
                        >
                            <span
                                class="rounded border border-slate-200 px-2 py-1 text-xs dark:border-slate-700"
                            >
                                {{ check.label }}
                            </span>
                            <Icon
                ...
                <div class="mt-4">
                    ...
                    <ol class="flex flex-wrap items-center gap-1.5">
                        <li
                            v-for="(check, index) in props.pipeline.text"
                            :key="check.key"
                            class="flex items-center gap-1.5"
                        >
                            <span
            ...
            >
                ...
                <p
                    class="mt-3 text-[11px] leading-relaxed text-slate-500 dark:text-slate-400"
                >
                    Without <span class="font-mono">ext-zip</span> archives
                    cannot be inspected; without
                    <span class="font-mono">ext-gd</span> images cannot be
                    decoded or re-encoded. Both checks report as unavailable
